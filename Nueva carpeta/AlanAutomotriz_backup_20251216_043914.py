...
class EjecutivoWindow:
    ...
    def registrar_pago(self):
        ...
        if pago_id == 'N/A':
            messagebox.showwarning("Advertencia", "⚠️ Primero debe establecer un precio")
            return

        # Obtener datos actuales
        self.db.cursor.execute('''
            SELECT monto_total, monto_pagado, historial_pagos 
            FROM pagos 
            WHERE id = ?
        ''', (pago_id,))
        ...
        monto_total, monto_pagado, historial_str = resultado
        pendiente = monto_total - monto_pagado

        # Validar monto de pago
        monto_pago_str = self.entry_monto_pago.get().strip()
        if not monto_pago_str:
            messagebox.showwarning("Advertencia", "⚠️ Ingrese el monto del pago")
            return
        ...
        historial.append(nuevo_registro_pago)
        ...
        try:
            ...
            self.db.cursor.execute('''
                ...
                SET monto_pagado = ?,
                    ...
                    notas = ?
                WHERE id = ?
            ''', (nuevo_pagado, nuevo_estado, monto_pago, metodo,
                  datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                  self.user_id, json.dumps(historial), notas, pago_id))
    ...
    def registrar_pago(self):
        ...
        try:
            # Registrar el pago
            self.db.cursor.execute('''
                        INSERT INTO pagos (ingreso_id  # CORREGIDO: antes era facturacion_id, monto, metodo_pago, registrado_por, notas)
                        VALUES (?, ?, ?, ?, ?)
